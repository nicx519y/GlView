export class Mesh {
	...
	private _borderVertexes: number[];
	private _unit: MeshUnit;
	private _bUnit: MeshUnit;
	/**
	 * 模型构造函数
	 * @param vertexes 顶点坐标
	 ...
	 * @param uv 材质UV
	 */
	constructor(mode: PrimitiveMode, vertexes: number[], borderVertexes: number[], uv: number[]) {
		this._vertexes = [];
		const clen = vertexes.length / 4;
		for(let i = 0; i < clen; i ++) {
			this._vertexes.push(vertexes[i*4], vertexes[i*4+1], vertexes[i*4+2], vertexes[i*4+3], 0, 0, uv[i*2], uv[i*2+1]);
		}
		this._borderVertexes = this.createBorderVertex(borderVertexes);
		this._unit = new MeshUnit(this._vertexes, mode);
		this._bUnit = new MeshUnit(this._borderVertexes, PrimitiveMode.TRIANGLE_STRIP);
	}
